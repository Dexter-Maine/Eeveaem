"""    
Basic Eeveaem server module for connecting users to E.V.M and RealmKoin Network.

Contains one class, EvmServer, which can be instantiated to start a server running
then used to send and receive messages from users as well as relaying mechanical interaction.
"""

from resources import *
from .CoreRooms import *
import socket
import select
import time
import sys
...
class C0reServer(object):    
    """    
    The Basic Object For Eeveaem Environment. 
    
    Once created, the server will listen for users connecting using Telnet. 
    Messages can then be sent to and from multiple connected users.
    ...
    class _Client(object):
        "Holds information about a connected user"
    ...
    def __init__(self):
        """    
        Constructs the C0reServer object and starts listening for new USERs.
        """
        ...
        # of the available network interfaces (Normal TESTING Environment: 0.0.0.0
        self._listen_socket.bind(("",23))
    ...
    def update(self):
        """    
        Checks for new USERs, disconnected USERs, and new messages sent from USERs.
        This method must be called before up-to-date info can be obtained from the 
        'get_new_USERs', 'get_disconnected_USERs' and 'get_commands' methods. 
        ...
        # with 'get_new_USERs', 'get_disconnected_USERs' and 'get_commands'. The
    ...
    def get_new_users(self):
        """    
        Returns a list containing info on any new USERs that have entered the game 
        since the last call to 'update'. Each item in the list is a USER id number.
        """
        retval = []
        # go through all the events in the main list
        for ev in self._events:
            # if the event is a new USER occurence, add the info to the list
            if ev[0] == self._EVENT_NEW_USER: retval.append(ev[1])
    ...
    def get_disconnected_users(self):
        """    
        Returns a list containing info on any USERs that have left the game since 
        the last call to 'update'. Each item in the list is a USER id number.
        """
        retval = []
        # go through all the events in the main list
        for ev in self._events:
            # if the event is a USER disconnect occurence, add the info to the list
            if ev[0] == self._EVENT_USER_LEFT: retval.append(ev[1])
        # return the info list
        return retval
    ...
    def get_commands(self):
        """    
        Returns a list containing any commands sent from USERs since the last call
        to 'update'. Each item in the list is a 3-tuple containing the id number of
        the sending USER, a string containing the command (i.e. the first word of 
    ...
    def send_message(self,to,message,FROM):
        """    
        Sends the text in the 'message' parameter to the USER with the id number 
        given in the 'to' parameter. The text will be printed out in the USER's
        terminal.
        """
        # we make sure to put a newline on the end so the client receives the
        # message on its own line
        self._attempt_send(to,'|[{}]|>>: '.format(FROM)+message+"\n\r")
    ...
    def _check_for_new_connections(self):
        ...
        self._clients[self._nextid] = C0reServer._Client(joined_socket,addr[0],"",time.time())

        # add a new USER occurence to the new events list with the USER's id 
        # number
        self._new_events.append((self._EVENT_NEW_USER,self._nextid))
        ...
        for id,cl in list(self._clients.items()):
            ...
            try:
                ...
                if message:
                    ...
                    gak,params = (message.split(" ",1)+["",""])[:2]
                    
                    # add a command occurence to the new events list with the 
                    # USER's id number, the command and its parameters
                    self._new_events.append((self._EVENT_COMMAND,id,gak.lower(),params))
    ...
    def _handle_disconnect(self,clid):
        ...
        # add a 'USER left' occurence to the new events list, with the USER's 
        # id number
        self._new_events.append((self._EVENT_USER_LEFT,clid))
